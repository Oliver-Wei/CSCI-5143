...
uint64_t green_toggle_count_delay = 20;
uint64_t yellow_toggle_delay = 0;
...
void initialize_system(void)
{
  ...
  // Red LED Task: timer 0, self-scheduled
	// Yellow LED Task: timer 3, prescaler 64, period 1 ms
  // Green LED Task: timer 1, prescaler 256, period 200 ms (actually 1000ms)
...
int main(void) {
  ...
  while(1) {
    ...
    */
    ...
    // temp_adc_results = adc_read(8);
    // if (temp_adc_results != adc_results){
    //   SET_BIT(*(&_yellow)->port, _yellow.pin);
    //   _delay_ms(5);
    //   TOGGLE_BIT(*(&_yellow)->port, _yellow.pin);
    //   adc_results = temp_adc_results;
    // }
